...
class Base:
    ...
    def __init__(self):
        self.base_dir = Config().get_option("path", "downloadfolder")
    ...
    def switch_vpn(turn):
        print("Switch VPN: {}\n".format(turn))
        vpn_name = Config().get_option("VPN", "name")
        vpn_user = Config().get_option("VPN", "user")
        vpn_password = Config().get_option("VPN", "password")
    ...
    def get_download_date():
        days = int(Config().get_option("main", "days"))
    ...
    @staticmethod
    def get_session():
        headers = {"User-Agent": Config().get_option("main", "useragent")}
    ...
    def get_download_failed_path(self):
        ...
        failed_dir = Config().get_option("path", "md5")
...
class StartMode:
    ...
    def __init__(self):
        mode_dict = {
            "malc0de": SampleMalc0de,
            "vxvault": SampleVxvault,
            "hybrid": SampleHybrid,
            "virusbay": SampleVirusBay,
            "malware-traffic-analysis": SampleMalwareTrafficAnalysis,
            "virusign": SampleVirusSign,
            "malshare": SampleMalshare,
            "infosec": SampleInfosec
        }
        for process in mode_dict:
            if Config().get_option("main", process) == "False":
                print(1)
                # mode_dict[process]()
